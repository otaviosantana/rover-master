...
class TestRotateLeft < Test::Unit::TestCase
  ...
  def test_return_west_when_rover_heading_to_north
    rover = Rover.new(Position.new(["0", "0", "N"]))
    Movement.new(0, 0).extend(RotateLeft).move rover
    assert_equal('W', rover.position.direction)
  end

  def test_return_south_when_rover_heading_to_west
    rover = Rover.new(Position.new(["0", "0", "W"]))
    Movement.new(0, 0).extend(RotateLeft).move rover
    assert_equal('S', rover.position.direction)
  end

  def test_return_east_when_rover_heading_to_south
    rover = Rover.new(Position.new(["0", "0", "S"]))
    Movement.new(0, 0).extend(RotateLeft).move rover
    assert_equal('E', rover.position.direction)
  end

  def test_return_north_when_rover_heading_to_east
